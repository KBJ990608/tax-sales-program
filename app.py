"""세무사 콜드메일 영업 프로그램 (Streamlit)

세무 정보를 직접 신청한 사장님들에게, 업체명·대표자가 들어간 맞춤 영업 메일을
합법적으로(수신 동의자에게만) 보내는 프로그램.
...
st.set_page_config(page_title="세무사 영업 메일 프로그램", page_icon="📧", layout="wide")
...
if page == "① 신청받기":
    st.header("① 신청받기")
    st.write("사장님이 세무 정보를 받겠다고 직접 신청하는 화면입니다. **수신 동의**에 체크해야 저장됩니다.")

    with st.form("apply", clear_on_submit=True):
        col1, col2 = st.columns(2)
        company = col1.text_input("업체명 *", placeholder="예: 행복상사")
        ceo = col2.text_input("대표자 이름", placeholder="예: 홍길동")
...
# ─────────────────────────────── ② 명단 관리 ───────────────────────────────
    ...
    st.header("② 명단 관리")
    st.write("잘못된 이메일·중복·이미 보낸 사람을 자동으로 구분해서 보여줍니다.")
    ...
    if not subs:
        ...
        del_email = st.selectbox(
            "삭제할 구독자 이메일", [""] + [r.get("이메일", "") for r in subs]
        )
...
# ─────────────────────────────── ③ 메일 보내기 ───────────────────────────────
    ...
    if mode == "고정 템플릿 5종":
        ...
        subj_tmpl = body_tmpl = None  # 고정 템플릿은 render()가 처리
    else:
        template_name = "AI맞춤"
        if not ai.has_key():
            st.warning(
    ...
    # 발송 대상 = 유효한 구독자 중 이 템플릿을 아직 안 받은 사람
    candidates = [
...
# ─────────────────────────────── ④ 발송 이력 ───────────────────────────────
    ...
    st.header("④ 발송 이력")
    st.write("누구에게 언제 어떤 메일을 보냈는지 기록입니다. 같은 사람에게 같은 템플릿은 다시 안 갑니다.")
